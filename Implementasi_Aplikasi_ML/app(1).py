...
<style>
    ...
    .header-heatmap {
        background: linear-gradient(135deg, #ad1457 0%, #6a1042 100%);
        border-radius: 16px; padding: 30px 40px; margin-bottom: 30px; color: white;
    }
    .header-heatmap h1 { font-size: 2.2rem; font-weight: 800; margin: 0; }
    .header-heatmap p  { font-size: 1rem; margin: 5px 0 0; opacity: 0.85; }

    .section-title-pink {
        font-size: 1.3rem; font-weight: 700; color: #f48fb1;
        border-left: 4px solid #ad1457; padding-left: 12px; margin: 25px 0 15px;
    }
    ...
    .badge-uploaded   { display:inline-block; background:#1b5e20; color:#a5d6a7; border-radius:6px; padding:2px 10px; font-size:0.8rem; font-weight:700; }
    .badge-missing    { display:inline-block; background:#3d0000; color:#ef9a9a; border-radius:6px; padding:2px 10px; font-size:0.8rem; font-weight:700; }
    .badge-processing { display:inline-block; background:#1a237e; color:#90caf9; border-radius:6px; padding:2px 10px; font-size:0.8rem; font-weight:700; }

    .heatmap-download-btn {
        background: linear-gradient(135deg, #ad1457, #6a1042);
        color: white; border: none; border-radius: 10px;
        padding: 12px 30px; font-weight: 700; font-size: 1rem;
        cursor: pointer; width: 100%;
    }
    .progress-bar-wrap {
        background: #2d3250; border-radius: 20px; height: 10px; margin: 8px 0;
        overflow: hidden;
    }
</style>
...
with st.sidebar:
    ...
    with sb1:
        ...
        if st.button("🌸 Heatmap\nGabungan", use_container_width=True):
            st.session_state.active_model = "Heatmap"
    with sb2:
    ...
    active_label = {
        ...
        "Heatmap": "🌸 Heatmap Gabungan",
    }.get(active, active)
    st.markdown(f"**Halaman Aktif:** {active_label}")
    st.markdown("---")
    ...
    if active == "GNB":
        ...
        st.markdown("#### 🌸 Upload Semua Dataset")
        st.markdown("Upload dataset masing-masing anggota untuk generate heatmap gabungan.")
        hm_heart    = st.file_uploader("💜 Heart Disease (CSV)",      type=["csv"],  key="hm_heart")
        hm_diabetes = st.file_uploader("💙 Diabetes SMOTE (CSV)",     type=["csv"],  key="hm_diabetes")
        hm_thal     = st.file_uploader("🩵 Thalassemia (XLSX)",       type=["xlsx"], key="hm_thal")
        hm_symptoms = st.file_uploader("💚 Gejala Diabetes (CSV)",    type=["csv"],  key="hm_symptoms")
        hm_ginjal   = st.file_uploader("🧡 Kanker Ginjal (CSV)",      type=["csv"],  key="hm_ginjal")
        hm_colormap = st.selectbox("🎨 Colormap Heatmap", ["RdYlGn","coolwarm","viridis","Blues","Spectral","YlOrRd"], key="hm_cmap")
        hm_dpi      = st.select_slider("📐 Resolusi PNG", options=[100, 150, 200, 300], value=150, key="hm_dpi")
        st.markdown("---")
        uploaded_count = sum(1 for f in [hm_heart, hm_diabetes, hm_thal, hm_symptoms, hm_ginjal] if f is not None)
        st.markdown(f"Dataset terupload: **{uploaded_count}/5**")

# ================================
# ============================================================
#   HALAMAN HEATMAP GABUNGAN
# ============================================================
# ================================
if st.session_state.active_model == "Heatmap":

    import io
    import base64

    st.markdown("""
    <div class="header-heatmap">
        <h1>🌸 Heatmap Korelasi Gabungan</h1>
        <p>Upload semua dataset preprocessing dari setiap anggota — semua heatmap korelasi ditampilkan & disimpan dalam satu file PNG</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    DATASETS = [
        ...
        },
        {
            ...
            "target":  "class",
            "type":    "csv",
            "numeric_only": False,   # perlu encode dulu
        },
        {
            "key":     "hm_ginjal",
            "label":   "🧡 Kanker Ginjal",
    ...
    # Ambil file dari sidebar (sudah di-render di atas)
    hm_files = {
        "hm_heart":    st.session_state.get("hm_heart",    None),
        "hm_diabetes": st.session_state.get("hm_diabetes", None),
        "hm_thal":     st.session_state.get("hm_thal",     None),
        "hm_symptoms": st.session_state.get("hm_symptoms", None),
        "hm_ginjal":   st.session_state.get("hm_ginjal",   None),
    }

    # Fallback: ambil dari widget langsung (cara Streamlit)
    ...
    # ── Status Upload ─────────────────────────────────────────
    st.markdown('<div class="section-title-pink">📋 Status Upload Dataset</div>', unsafe_allow_html=True)

    status_cols = st.columns(5)
    loaded_data = {}   # key → DataFrame

    for i, ds in enumerate(DATASETS):
        f = _get_upload(ds["key"])
        with status_cols[i]:
            st.markdown(f"""
            <div class="upload-card">
                <div class="uc-title" style="color:{ds['color']}">{ds['label']}</div>
                <div class="uc-sub">{ds['owner']} · {ds['model']}</div>
            </div>""", unsafe_allow_html=True)
            if f is not None:
                st.markdown('<span class="badge-uploaded">✅ Terupload</span>', unsafe_allow_html=True)
            else:
                st.markdown('<span class="badge-missing">⏳ Belum upload</span>', unsafe_allow_html=True)
    ...
    for ds in DATASETS:
        ...
        if f is not None:
            try:
                ...
                # Encode kolom object jika diperlukan
                df_enc = df_tmp.copy()
                for col in df_enc.columns:
                    if df_enc[col].dtype == object:
                        # Yes/No
                        vals = set(df_enc[col].dropna().astype(str).str.strip().str.lower().unique())
                        if vals <= {'yes','no'}:
                            df_enc[col] = df_enc[col].astype(str).str.strip().str.lower().map({'yes':1,'no':0})
                        elif col.lower() == 'gender':
                            df_enc[col] = df_enc[col].astype(str).str.strip().str.lower().map({'male':1,'female':0,'m':1,'f':0})
                        elif col.lower() in ['class','target','outcome','diagnosis']:
                            try:
                ...
                df_num = df_enc.select_dtypes(include=[np.number]).dropna(axis=1, how='all')
                loaded_data[ds["key"]] = {"df": df_num, "meta": ds, "raw": df_tmp}
            except Exception as e:
                st.warning(f"⚠️ Gagal memuat {ds['label']}: {e}")

    n_loaded = len(loaded_data)

    # ── Jika belum ada yang diupload ─────────────────────────
    if n_loaded == 0:
        st.markdown("""
        <div class="empty-state">
            <h2>📂 Belum Ada Dataset yang Diupload</h2>
            <p>Upload minimal 1 dataset melalui sidebar kiri untuk mulai generate heatmap</p>
        </div>""", unsafe_allow_html=True)
        st.stop()

    # ── Info summary ─────────────────────────────────────────
    st.markdown(f'<div class="section-title-pink">📊 {n_loaded} Dataset Siap — Preview</div>', unsafe_allow_html=True)
    prev_cols = st.columns(min(n_loaded, 3))
    for i, (key, val) in enumerate(loaded_data.items()):
        with prev_cols[i % 3]:
            meta = val["meta"]
            df_p = val["df"]
            st.markdown(f"**{meta['label']}** — {meta['owner']}")
            st.caption(f"{val['raw'].shape[0]} baris · {val['raw'].shape[1]} kolom asli · {df_p.shape[1]} fitur numerik")
            st.dataframe(df_p.head(4), use_container_width=True)

    # ── Generate gabungan heatmap ─────────────────────────────
    st.markdown('<div class="section-title-pink">🔥 Heatmap Korelasi — Semua Dataset</div>', unsafe_allow_html=True)

    colormap_choice = st.session_state.get("hm_cmap", "RdYlGn")
    dpi_choice      = st.session_state.get("hm_dpi", 150)

    # Layout: maksimal 3 kolom per baris
    MAX_COLS   = 3
    n_plots    = n_loaded
    n_rows_fig = (n_plots + MAX_COLS - 1) // MAX_COLS
    n_cols_fig = min(n_plots, MAX_COLS)

    # Ukuran setiap heatmap
    CELL_W = 7
    CELL_H = 6

    fig, axes = plt.subplots(
        n_rows_fig, n_cols_fig,
        figsize=(CELL_W * n_cols_fig, CELL_H * n_rows_fig)
    )
    ...
    # Gambar setiap heatmap
    for idx, (key, val) in enumerate(loaded_data.items()):
        row = idx // MAX_COLS
        col = idx %  MAX_COLS
        ax  = axes[row][col]
        ...
        # Gunakan colormap per-dataset atau pilihan user
        use_cmap = colormap_choice if colormap_choice != "RdYlGn" else meta["cmap"]
        ...
        sns.heatmap(
            ...
            linewidths=0.4,
            linecolor='#1a1d2e',
            annot_kws={"size": max(5, min(9, 80 // max(len(df_h.columns), 1)))},
            cbar_kws={"shrink": 0.8},
            square=True,
            vmin=-1, vmax=1,
        )

        # Title dengan warna sesuai model
        ax.set_title(
            f"{meta['label']}\n{meta['owner']} — {meta['model']} | {val['raw'].shape[0]} data",
            color='white',
            fontweight='bold',
            fontsize=11,
            pad=10
        ...
        for spine in ax.spines.values():
            spine.set_color(meta["color"])
            spine.set_linewidth(2)

        # Colorbar labels warna putih
    ...
    for idx in range(n_plots, n_rows_fig * n_cols_fig):
        row = idx // MAX_COLS
        col = idx %  MAX_COLS
        axes[row][col].set_visible(False)

    # Judul besar gabungan
    fig.suptitle(
        "🌸  Heatmap Korelasi Dataset — Kelompok Klasifikasi Penyakit",
        color='white',
        fontsize=15,
        fontweight='bold',
        y=1.01
    ...
    # ── Simpan ke PNG dan buat tombol download ────────────────
    st.markdown('<div class="section-title-pink">⬇️ Download Heatmap Gabungan (PNG)</div>', unsafe_allow_html=True)
    ...
    st.caption(f"📄 Ukuran file: **{file_size_kb:.1f} KB** · Resolusi: **{dpi_choice} DPI** · {n_loaded} heatmap dalam 1 PNG")

    st.download_button(
        label="⬇️ Download heatmap_gabungan.png",
        data=img_bytes,
        file_name="heatmap_gabungan.png",
        mime="image/png",
        use_container_width=True,
    )

    # ── Statistik ringkas setiap dataset ─────────────────────
    st.markdown('<div class="section-title-pink">📋 Ringkasan Korelasi Tertinggi per Dataset</div>', unsafe_allow_html=True)

    for key, val in loaded_data.items():
        meta = val["meta"]
        df_h = val["df"]
        corr = df_h.corr()
        # Ambil 3 pasang korelasi tertinggi (abs), kecuali diagonal
        corr_pairs = (
        ...
        top3 = corr_pairs.sort_values('|Korelasi|', ascending=False).head(3)

        with st.expander(f"{meta['label']} — Top 3 Korelasi Tertinggi"):
            c1, c2 = st.columns([1, 2])
            with c1:
                st.metric("Total Fitur",  df_h.shape[1])
                st.metric("Total Data",   val["raw"].shape[0])
                st.metric("Korelasi Maks", f"{corr_pairs['|Korelasi|'].max():.3f}")
            with c2:
                st.dataframe(
                    top3[['Fitur A','Fitur B','Korelasi']].reset_index(drop=True).round(4),
                    use_container_width=True
                )
...
# ============================================================
# ================================
elif st.session_state.active_model == "GNB":
